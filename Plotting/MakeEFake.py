...
#   closure( sampManElG, samp, sel_base_el,'EB', plot_var = 'm_lep_ph',varbins=[-2.4,-1.8,-1.4,-1,-0.6,-0.3,0,0.3,0.6,1,1.4,1.8,2.4], var="ph_eta")
    ...
    #closure( sampManElG, samp, sel_base_el,'EB', plot_var = 'm_lep_ph',varbins=[20,30,40,50,70,90,110,150,160], mode=1)
...
def closure( sampMan, sample, sel_base, eta_cut, var="ph_pt", varbins = range(0,200,25),xtitle="photon p_{T}", plot_var='m_lep_ph', suffix='', workspace=None, overlaprm=1,mode=1) :
    ...
    for i in range(len(varbins)-1):
        ...
        if options.data:
            ...
            sga=np.array(s);sga[2]=zg[2]+wg[2]
            zga=np.array(zg)
            wga=np.array(wg)
            sga_sub = sga-zga-wga
            print 'Signal,Zg,Wg: ',s,zg,wg
            print 'pollution: ',(zga+wga)/sga
            print 'subtracted: ',sga_sub
            sr_est, sr_est_e, sr_sum, sr_sum_e = calABD(*sga_sub)
            ...
            sa,sb,ss,sd= closureBin(sampMan, sample, sel_base1, eta_cut, plot_var, overlaprm,filename,mode=mode,doSave=1) 
